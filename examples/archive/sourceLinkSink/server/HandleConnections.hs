...
---- experimental - aim is to replace sendSorce so allowing the source to do some local processing before sending off events to other partitions
handleSource :: Show beta => IO alpha -> (Stream alpha -> Stream beta) -> IO ()
handleSource pay streamGraph = do
